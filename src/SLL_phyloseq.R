...
} else if (dir.exists("/home/jwillis/gpfs/projects/bsc40/jwillis/SLL")) {
  home_dir <- "/home/jwillis/gpfs/projects/bsc40/jwillis/SLL"
...
if (dir.exists("/users/tg/jwillis/SLL") || dir.exists("/home/jwillis/gpfs/projects/bsc40/jwillis/SLL") ||
    dir.exists("/gpfs/projects/bsc40/current/jwillis/SLL") || dir.exists(sprintf("%s/SLL", getwd())) || dir.exists("~/Downloads/SLL")) {
...
# SLL <- prune_samples( sample_sums(SLL) >= 1000, SLL)
...
# saveRDS(as.data.frame(as.matrix(jsd)), file = sprintf("%s/R_objects/beta_diversities/SLL1_jsd.mothur.rds", p1_dir))
# 
# # *********************** #
jsd <- readRDS(sprintf("%s/R_objects/beta_diversities/SLL1_jsd.mothur.rds", p1_dir))
...
# for request from people at dbBact from Sheba medical institute in Israel
sllmetas <- microbiome::meta(SLL)
sllmetas <- sllmetas[ , c("Q29","Q30","Q32","Q37","Q39.1",#"Q39.2",
                          "Q40.1",#"Q40.2",
                          "Q41","Q00.PH","water_pH",
                          "Q22.1","Q24.1","Q25.1")]
colnames(sllmetas) <- c("Superficial_repairs","Braces","Dry_mouth","Fluoride_toothpaste",
                        "Do_you_brush","Do_you_floss","Mouth_wounds.6_months","pH.Saliva","pH.water",
                        "Milk","Sweets","Gum")
for (col in colnames(sllmetas)) {
  print(col)
  print(table(sllmetas[,col]))
  print(sum(table(sllmetas[,col])))
  sllmetas[,col] <- sapply(as.character(sllmetas[,col]), function(x) ifelse(x=="No Sabe/No Contesta","",x))
  
  if (col %in% c("Braces","Dry_mouth","Fluoride_toothpaste",
                 "Do_you_brush","Do_you_floss","Mouth_wounds.6_months",
                 "Milk","Sweets","Gum")) {
    sllmetas[,col] <- sapply(as.character(sllmetas[,col]), function(x) ifelse(x=="0","No",
                                                                              ifelse(x=="1","Yes",x)))
  }
  print(table(sllmetas[,col]))
  print("")
...
write.csv(sllmetas, file = "/home/jwillis/gpfs/projects/bsc40/jwillis/SLL/Part_1/SLL_metadata.csv")
...
# table_of_interest <- gen_otu_table; tlev <- "Género"
# 
...
# SLL1.pie["Otros",] <- colSums(table_of_interest[nottop10,])
# 
# 
# 
...
# 
# 
# # use same colors as in SLL2 stacked bar charts
# library(RColorBrewer)
# library(ggpubr)
# getPalette = colorRampPalette(brewer.pal(12,"Paired"))
# # colPal.set2=rev(getPalette(19))
# colPal=c("WhiteSmoke",rev(getPalette(15)))
# colPal.sll1 <- colPal[c(1,16,15,13,12,14)]
# 
# #plot
...
#   scale_fill_manual(values = colPal.sll1) + theme_void() +
#   labs(fill=tlev) + ggtitle(community) + theme(plot.title = element_text(hjust=0.5)) +
#   ggsave(sprintf("/users/tg/jwillis/SLL/Part_1/figures/Abundances/pie_charts/Genero/%s_genero.png", 
#                  gsub(' ','_',community)), device="png")
